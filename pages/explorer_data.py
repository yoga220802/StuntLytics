...
def render_page():
    ...
    with c4:
        jam = st.select_slider(
            "Jamban Sehat", options=["Semua", "Ada", "Tidak"], value="Semua"
        )

    # filter lanjutan
    dfe = df_filtered.copy()
    if edu:
        dfe = dfe[dfe["pendidikan_ibu"].isin(edu)]
    if asi != "Semua":
        dfe = dfe[dfe["asi_eksklusif"] == (1 if asi == "Ya" else 0)]
    if air != "Semua":
        dfe = dfe[dfe["akses_air_layak"] == (1 if air == "Ada" else 0)]
    if jam != "Semua":
        dfe = dfe[dfe["jamban_sehat"] == (1 if jam == "Ada" else 0)]

    st.caption(
        "Tabel dapat di-scroll & difilter. Gunakan ikon search di kanan atas tabel."
    )
    st.dataframe(
        dfe[
            [
        ...
        # FIX: Replaced deprecated parameter
        width="stretch",
        height=420,
    )
