...
def main():
    ...
    with col1:
        ...
        st.metric("", f"{total_observasi:,}")
        st.markdown('<div class="small-muted">setelah filter</div></div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        """, unsafe_allow_html=True)
        st.metric("", f"{pct_high:.1f}%")
        st.markdown('<div class="small-muted">target ↓ per triwulan</div></div>', unsafe_allow_html=True)
    ...
    with col3:
        ...
        """, unsafe_allow_html=True)
        st.metric("", f"{imun_cov:.1f}%")
        st.markdown('<div class="small-muted">indikator kunci SSGI</div></div>', unsafe_allow_html=True)
    ...
    with col4:
        ...
        """, unsafe_allow_html=True)
        st.metric("", f"{air_cov:.1f}%")
        st.markdown('<div class="small-muted">sektor WASH</div></div>', unsafe_allow_html=True)
        
    st.info("Selamat datang di Dashboard StuntLytics. Gunakan navigasi di sebelah kiri untuk menjelajahi fitur-fitur analisis.", icon="👋")
